...
**Version described: 2.2.0.** This document describes what the app is, how every
...
plus `https://github.com/Starf0x/my-audiobook-collection` at tag `v2.2.0` is an
...
| `unreachable-tailscale` | the DNS message names the Tailscale setting by the name Unraid gives it, says why that resolver cannot answer, still names the custom-network case, and leaves Tailscale out of every other failure |
